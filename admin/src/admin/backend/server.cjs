...
app.post("/generate-audio", async (req, res) => {
  try {
    ...
    const textHash = require("crypto")
      .createHash("md5")
      .update(cleanText.substring(0, 100))
      .digest("hex")
      .substring(0, 8);
    ...
    console.log(
      `Text preview: "${cleanText.substring(0, 150)}${cleanText.length > 150 ? "..." : ""}"`,
    );
    ...
    console.log(
      `Looking for existing translation: artifact_id=${artifactId}, language_code=${langCode}`,
    );
    ...
    if (artifactError || !artifactCheck) {
      throw new Error(
        `Artifact not found: ${artifactId}. Error: ${artifactError?.message}`,
      );
    }
    ...
    if (existingTx) {
      console.log(
        `Updating existing translation ${existingTx.id} with audio_url`,
      );
    ...
    if (verifyError || !verification?.audio_url) {
      console.log(
        "Direct save failed, attempting fallback: fetch from bucket and save",
      );
      ...
      if (listError || !bucketFiles?.length) {
        throw new Error(
          `No audio files found in bucket for artifact ${artifactId}, language ${lang}`,
        );
      }
      ...
      console.log(
        "✅ Audio URL successfully saved and verified:",
        verification.audio_url,
      );
    }
  ...
  try {
    ...
    res.json({
      ...
      changes: {
        ...
        percentageRemoved: (
          ((text.length - sanitized.length) / text.length) *
          100
        ).toFixed(1),
...
app.post("/generate-all-audio", async (req, res) => {
  try {
    const { artifactId, descriptions, voiceName, speakingRate, skipSanitize } =
      req.body;
    ...
    for (const [lang, text] of Object.entries(descriptions)) {
      ...
      try {
        const response = await fetch(
          `http://localhost:${PORT}/generate-audio`,
          {
            method: "POST",
...
app.listen(PORT, () => {
  ...
  console.log("\n Available endpoints:");
  console.log(
    "  POST   /generate-audio        - Generate audio (auto-sanitizes descriptions)",
  );
